...
function BodePlot(mags, mags_min, mags_max, phases, phases_min, phases_max, mag_plot_div, phase_plot_div){
    ...
    this.redraw = function(){
        ...
        this.dataPhs = [];
        this.magDataPoints = [];
        this.phaseDataPoints = [];
    
        //Push magnitude and phase data into graph.
        for(var i = 0; i < range.length; i++){
            this.phaseDataPoints.push({
                x: range[i],
                y: this.phases[i]
            });
        
            this.magDataPoints.push({
                x: range[i],
                y: this.mags[i]
            });
        }
    
        this.dataMag.push({data: this.magDataPoints, width: 1, color: 'blue', stroke: "0,0", legend: "Magnitude" });
    
        this.seriesMag = this.plotMag.selectAll(".line").data(this.dataMag);
    
        this.seriesMag.enter().append("path");
    
        this.seriesMag.attr("class","line")
            .attr("d",function(d) { return this.magLine(d.data); })
            .attr("stroke-width", function(d) { return d.width; })
            .style("stroke", function(d) { return d.color; })
            .style("stroke-dasharray", function(d) { return d.stroke; });
    
        this.dataPhs.push({data: this.phaseDataPoints, width: 1, color: 'blue', stroke: "0,0", legend: "Phase" });
    
        this.seriesPhs = this.plotPhs.selectAll(".line").data(this.dataPhs);
    
        this.seriesPhs.enter().append("path");
    
        this.seriesPhs.attr("class","line")
            .attr("d",function(d) { return this.phsLine(d.data); })
            .attr("stroke-width", function(d) { return d.width; })
            .style("stroke", function(d) { return d.color; })
            .style("stroke-dasharray", function(d) { return d.stroke; });
    
        this.plotPhs.selectAll('.x1.axis>.tick')  // find all the x axis ticks and loop
            .each(function(d,i){
...
output_impedance_bode.redraw();
//input_output_bode.createPlots();
//duty_output_bode.createPlots();